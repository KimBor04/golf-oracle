...
OUTPUT_PATH = PREDICTIONS_DIR / "leaderboard.parquet"
...
def predict_round1(model, inference_df: pd.DataFrame) -> pd.DataFrame:
    ...
    predictions = predictions.sort_values("predicted_round1", ascending=True).reset_index(drop=True)
...
def print_event_summary(predictions_df: pd.DataFrame) -> None:
    event_mae = predictions_df["abs_error"].mean()
    event_rmse = ((predictions_df["predicted_round1"] - predictions_df["actual_round1"]) ** 2).mean() ** 0.5

    print("\n=== EVENT BACKTEST SUMMARY ===")
    print(f"Tournament: {predictions_df['target_tournament'].iloc[0]}")
    print(f"Start date: {predictions_df['target_start'].iloc[0].date()}")
    print(f"Players evaluated: {len(predictions_df)}")
    ...
    print(
        predictions_df.sort_values(["actual_rank", "player_name_clean"])[
...
def main() -> None:
    ...
    print_event_summary(predictions_df)
